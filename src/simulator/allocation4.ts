...
type Stage = 'first_job' | 'second_job' | 'future';
type SkillRole = 'attack' | 'mastery' | 'speed' | 'accuracy' | 'survival' | 'mobility' | 'aoe' | 'utility';
...
interface BuildConfig {
  ...
  firstSkills: SkillDef[];
  secondSkills: SkillDef[];
}
...
function skill(name: string, stage: Stage, max: number, unlock: number, priority: number, role: SkillRole, reason: string, effect: SkillEffect): SkillDef {
...
const FIRST_SKILLS: Record<JobProfile['family'], SkillDef[]> = {
  warrior: [
    skill('Power Strike', 'first_job', 20, 10, 100, 'attack', '一转先加主攻，提高前期单体击杀速度。', { skillMult: 0.025 }),
  ...
  magician: [
    skill('Magic Claw', 'first_job', 20, 15, 104, 'attack', '一转稳定主力输出。', { magicSkillDamage: 2.15, skillMult: 0.008 }),
    skill('Energy Bolt', 'first_job', 20, 10, 90, 'attack', '前期过渡主攻。', { magicSkillDamage: 1.8 }),
  ...
  archer: [
    skill('Double Shot', 'first_job', 20, 10, 100, 'attack', '前期主攻技能。', { skillMult: 0.022 }),
    skill('The Eye of Amazon', 'first_job', 8, 10, 90, 'accuracy', '射程和命中优先，减少效率损失。', { accuracy: 2.4, rangeScore: 0.025 }),
    skill('Critical Shot', 'first_job', 20, 18, 78, 'attack', '提高平均伤害。', { critRate: 0.006, critDamage: 0.7 }),
    skill('Focus', 'first_job', 20, 24, 52, 'accuracy', '后补命中和回避。', { accuracy: 1.4, avoid: 1.2 }),
  ],
  rogue: [
    skill('Lucky Seven / Double Stab', 'first_job', 20, 10, 100, 'attack', '职业核心主攻技能。', { skillMult: 0.024 }),
  ...
  pirate: [
    skill('Somersault Kick / Double Shot', 'first_job', 20, 10, 100, 'attack', '海盗预留主攻技能。', { skillMult: 0.023 }),
...
const SECOND_SKILLS: Record<JobKey, SkillDef[]> = {
  fighter: [
    skill('Sword Mastery', 'second_job', 20, 30, 112, 'mastery', '二转优先补 mastery，稳定最小伤害。', { mastery: 0.032 }),
    skill('Sword Booster', 'second_job', 20, 34, 92, 'speed', '攻速提高，直接提高每小时击杀数。', { attackInterval: -0.01 }),
    skill('Rage', 'second_job', 20, 38, 86, 'utility', '攻击力提升，放大后续练级收益。', { weaponAttack: 0.9, attackPower: 0.45 }),
    skill('Power Guard', 'second_job', 30, 45, 58, 'survival', '后补生存，降低高密度图药耗。', { survivability: 0.008, potionIntensity: -0.005 }),
  ],
  page: [
    skill('Sword Mastery', 'second_job', 20, 30, 108, 'mastery', '先补 mastery，保证伤害稳定。', { mastery: 0.032 }),
    skill('Sword Booster', 'second_job', 20, 34, 86, 'speed', '提高攻击频率。', { attackInterval: -0.009 }),
  ...
  spearman: [
    ...
    skill('Spear Booster', 'second_job', 20, 34, 92, 'speed', '攻速提升，改善每小时击杀数。', { attackInterval: -0.01 }),
  ...
  fire_poison: [
    skill('Fire Arrow', 'second_job', 30, 30, 112, 'attack', '火毒核心输出。', { magicSkillDamage: 2.8, skillMult: 0.01 }),
  ...
  ice_lightning: [
    ...
    skill('Cold Beam', 'second_job', 30, 34, 104, 'attack', '单体输出兼顾控场。', { magicSkillDamage: 2.4, survivability: 0.004 }),
  ...
  cleric: [
    ...
    skill('Holy Arrow', 'second_job', 30, 45, 54, 'attack', '后补单体输出。', { magicSkillDamage: 1.55 }),
  ],
  hunter: [
    skill('Bow Mastery', 'second_job', 20, 30, 112, 'mastery', '先补 mastery，稳定远程输出。', { mastery: 0.032, accuracy: 0.6 }),
    skill('Bow Booster', 'second_job', 20, 34, 92, 'speed', '攻速提升。', { attackInterval: -0.01 }),
  ...
  crossbowman: [
    ...
    skill('Crossbow Booster', 'second_job', 20, 34, 92, 'speed', '攻速提升。', { attackInterval: -0.01 }),
  ...
  assassin: [
    ...
    skill('Claw Booster', 'second_job', 20, 34, 92, 'speed', '攻速提高。', { attackInterval: -0.01 }),
    skill('Critical Throw', 'second_job', 30, 38, 102, 'attack', '刺客核心暴击收益。', { critRate: 0.007, critDamage: 0.9 }),
  ...
  bandit: [
    ...
    skill('Dagger Booster', 'second_job', 20, 34, 90, 'speed', '攻速提升。', { attackInterval: -0.009 }),
    skill('Savage Blow', 'second_job', 30, 38, 112, 'attack', '侠客二转核心爆发。', { skillMult: 0.018 }),
  ...
  brawler: [
    ...
    skill('Knuckle Booster', 'second_job', 20, 34, 92, 'speed', '攻速提升。', { attackInterval: -0.01 }),
    skill('Corkscrew Blow', 'second_job', 20, 38, 86, 'attack', '位移攻击提高效率。', { skillMult: 0.014, mobilityScore: 0.01 }),
  ...
  gunslinger: [
    ...
    skill('Gun Booster', 'second_job', 20, 34, 92, 'speed', '攻速提升。', { attackInterval: -0.01 }),
...
function buildConfig(job: JobProfile): BuildConfig {
  const family = job.family;
  const primary: Record<JobProfile['family'], StatKey> = {
    warrior: 'str', magician: 'int', archer: 'dex', rogue: 'luk', pirate: job.key === 'gunslinger' ? 'dex' : 'str',
  };
  const secondary: Record<JobProfile['family'], StatKey> = {
    warrior: 'dex', magician: 'luk', archer: 'str', rogue: 'dex', pirate: job.key === 'gunslinger' ? 'str' : 'dex',
  };
  const secondaryTarget = (level: number) => {
    if (family === 'warrior') return Math.max(35, level * 2);
    if (family === 'magician') return level + 3;
    if (family === 'rogue') return Math.floor(level * 2.1);
    return level + 5;
  };
  return { family, primary: primary[family], secondary: secondary[family], secondaryTarget, firstSkills: FIRST_SKILLS[family], secondSkills: SECOND_SKILLS[job.key] };
}

function adjustedPriority(skillDef: SkillDef, strategy: Strategy): number {
  let score = skillDef.priority;
  if (strategy === 'fastest' && ['attack', 'aoe', 'mastery', 'speed'].includes(skillDef.role)) score += 18;
  if (strategy === 'safe' && ['survival', 'accuracy', 'mobility'].includes(skillDef.role)) score += 18;
  if (strategy === 'profit' && ['utility', 'survival'].includes(skillDef.role)) score += 14;
  if (strategy === 'balanced' && ['mastery', 'speed'].includes(skillDef.role)) score += 8;
  return score;
}
...
function allocateSp(level: number, config: BuildConfig, strategy: Strategy, skillLevels: Record<string, number>): SkillPointAllocation[] {
  ...
  const pool = [...(stage === 'first_job' ? config.firstSkills : []), ...(stage === 'second_job' ? config.secondSkills : [])]
  ...
  while (remaining > 0 && pool.length) {
    pool.sort((a, b) => adjustedPriority(b, strategy) - adjustedPriority(a, strategy));
...
function derive(level: number, job: JobProfile, config: BuildConfig, stats: CharacterStats, skillLevels: Record<string, number>): DerivedCombatStats {
  ...
  const derived: DerivedCombatStats = {
  ...
  for (const skillDef of [...config.firstSkills, ...config.secondSkills]) {
    const skillLevel = skillLevels[skillDef.name] ?? 0;
    if (skillLevel > 0) addEffect(derived, skillDef.effect, skillLevel);
  }
...
export function generateAllocationPlan(jobKey: JobKey, startLevel: number, targetLevel: number, strategy: Strategy): AllocationPlan & { snapshots: Record<number, CharacterBuildSnapshot> } {
  ...
  const stats = baseStats(job.family);
